...
This runs every test except the real-browser integration test, which is skipped unless credentials are supplied (see below) — so the default run is fast (a couple of seconds for ~160 test cases, most of them table-driven subtests) and needs no network access or browser. CI runs it as `go test ./... -race -cover`.

**Coverage** (`go test -race -coverprofile=cover.out -coverpkg=./... ./... && go tool cover -func=cover.out`): every function in `browser/login.go`, `browser/policies.go`, `browser/documents.go`, `browser/session.go`, `browser/options.go` and `browser/client.go` (the actual challenge logic) is at **100%**, as is `run()` in `cmd/carrierproxy/main.go` (tested with a fake `PolicyProvider`; only the three-line `main()` that wires in the real environment and browser is left out). The default run comes to about 78% overall; the gap is the go-rod adapter in `browser/rod.go`, which needs a real browser to exercise, plus one `cookiejar.New` error branch in `browser/http.go` that the standard library never actually takes.

To cover that adapter too, and to fulfil "test(s) ... that accept environment variables for the credentials": set `CARRIERPROXY_USERNAME`/`CARRIERPROXY_PASSWORD` and run the suite again (about 93% overall with this included — the remaining gap is `main()` and the defensive branches in the go-rod adapter, e.g. cleanup after an already-failed launch or a browser that refuses to close, that a successful run has no reason to hit):
